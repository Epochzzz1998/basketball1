...
> ESPN 这条路后来被 B-R 取代（ESPN 季后赛有真实缺口、日期是 UTC）。季后赛见
> `po_game_logs_br.py`，常规赛见下面这一节。

## 常规赛逐场（rs_game_logs_br.py）

和季后赛同源同解析器——B-R 的 box score 页两者结构一致，`parse_box_table` 直接 import 复用，
不同的只是**怎么枚举比赛**：季后赛走系列赛页，常规赛走 `/leagues/{LG}_{year}_games.html` 的逐月分页。

```bash
./run_rs_games.sh                                  # 1947-2026，从新到旧，两阶段串起来
python3 rs_game_logs_br.py --status                # 随时看进度
python3 rs_game_logs_br.py --build --seasons 2026  # 抓完之后入库
```
...
| 阶段 | 产物 | 说明 |
|---|---|---|
| `--index` | `rs_index_cache/{year}.json` | 每季约 9 个请求，列出该季全部常规赛场次 |
| `--scrape` | `rs_games_cache/{year}.jsonl` | **一场一行、逐场 flush**，中断只亏一个请求 |
| `--build` | `rs_game_logs_{year}.sql` | 按季生成并入库（150 万行不能塞进一个文件） |

三件必须知道的事：

1. **附加赛和季中锦标赛决赛要剔掉。** 逐月页里混着季后赛（4 月 2025：149 场里 38 场是季后赛），
   而附加赛和 NBA Cup 决赛**两边都不计入统计**——混进来，逐场累加就和 `player_stats` 的赛季场均对不上。
   季后赛按 id 排除（和季后赛数据集的 id 完全一致）；附加赛看 B-R 的 `game_remarks` 列；
   Cup 决赛 B-R **不单独标注**，靠"当季最后一场 Cup 赛"识别。2025-26：1237 → 1230，正好。
2. **交易赛季的 `PLAYER_TEAM` 是 `HOU->BKN`**，认人时要拆开，两支球队都注册，否则被交易的球员整季无法匹配。
3. **B-R 的老比赛个人数据是逐场稀疏的**，不是按赛季一刀切：1963 抽样三场都全，1966–1969 有满有空，
   1971 起稳定。没有数据的比赛就是 0 行，不算错。

认人四步：全名 → 去后缀 → B-R slug → 首字母+姓。最后一步只在**单队单季**的池子里用，
且同一个 key 有两个人认领时直接丢弃（认错人比认不出更糟）。另外做了**同形字折叠**：
B-R 把 Egor Dёmin 写成西里尔字母 ё，NFD 去掉分音符后剩下的是西里尔 е，和拉丁 e 长得一样但匹配不上。
