...
export default function TopicsList() {
   ...
   */
  const isFav = (x) => !!(x.subscribed || x.pinned)

  // 筛选器只列**真的有专题挂着**的类别：配了但没人用的类别摆在那儿只会点出一片空
  const catOptions = useMemo(() => {
    if (!topics?.length) return []
    const count = (id) => topics.filter((x) => (x.categoryId || '') === id).length
  ...
  const shown = useMemo(() => {
    ...
    const hit = cat === 'all' ? topics
      : cat === 'fav' ? topics.filter(isFav)
        : topics.filter((x) => (x.categoryId || '') === cat)
    const k = kw.trim().toLowerCase()
    if (!k) return hit
    // 专题名 + 简介 + 类别名一起匹配（列表已全量在手，纯前端筛）
    return hit.filter((x) => `${x.name || ''}${x.description || ''}${x.categoryName || ''}`.toLowerCase().includes(k))
  }, [topics, cat, kw])
      ...
      {shown === null ? (
        ...
        <Row gutter={[16, 16]}>
          {shown.map((tp) => {
            ...
            return (
              <Col key={tp.topicId} xs={24} sm={12} lg={8}>
                ...
                {tp.newCount > 0 && (
                  ...
                  >
                    {tp.newCount > 99 ? '99+' : tp.newCount}
                  </span>
                )}
                <Card
                  className="topic-card"
                  style={{ borderRadius: 14, height: '100%', overflow: 'hidden', cursor: tp.locked ? 'default' : 'pointer', opacity: tp.locked ? 0.85 : 1 }}
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 17, fontWeight: 800, color: art ? '#fff' : undefined, textShadow: shadow, ...clamp(1) }}>{tp.name}</div>
                      {/* 状态标记：一行裸图标，和右上角那排操作图标同一套语言。
                          原来是四五枚彩色 Tag，能占两行——一张卡上最抢眼的成了这些次要属性 */}
                      <TopicBadges topic={tp} light={art} style={{ marginTop: 5 }} />
                    </div>
                  ...
                  <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', fontSize: 12, color: art ? 'rgba(255,255,255,.9)' : '#999', textShadow: shadow, minHeight: 32 }}>
                    ...
                    {tp.chatUnread > 0 && (
                      ...
                      >
                        ...
                        {tp.chatUnread > 99 ? '99+' : tp.chatUnread}
                      </span>
                    )}
                    <span style={{ flex: 1 }} />
