  ...
  return (
    ...
    >
      <div style={{ fontSize: 13, color: '#8c8c8c', marginBottom: 12 }}>
        {t("打包成 zip：")}<code>messages.json</code>{t("（结构化）、")}<code>chat.txt</code>{t("（可读流水）、")}
        <code>files/</code>{t("（图片和附件原件）。")}
      </div>
      <Checkbox checked={all} onChange={(e) => setAll(e.target.checked)}>{t("导出全部记录")}</Checkbox>
      ...
      {!all && range && (
        <Alert
          style={{ marginTop: 12 }}
          type="info"
          message={t("导出 {{v0}} 到 {{v1}}，含 {{v2}} 当天到此刻的全部消息", { v0: range[0].format(t('M 月 D 日')), v1: range[1].format(t('M 月 D 日')), v2: range[1].format(t('M 月 D 日')) })}
        />
      )}
    </Modal>
  )
